var Class = require('./search/class').Class,
	PorterStemmer = require('./search/stemmer/porter').Stemmer,
	DefaultTokenizer = require('./search/tokenizer/default').Tokenizer,
	DoubleMetaphone = require('./search/double-metaphone').DoubleMetaphone,
	Vector = require('./search/vector').Vector;
...
var Search = new Class({
	...
	index: function(documents){
		...
		return documents.forEach(function(doc){
			...
			// accepts an array or a forEachable that returns a promise to make it async...
			// if you're into that kinda thing
			if( typeof(doc) == "object" ) {
				for( var field in doc ){
					this.fields.push(field);
					var vector = makeVector(doc[field],this.fieldWeights[field]||0,this.vectorKeywordIndex);
				...
				this.documentVectors.push(makeVector(doc,0,this.vectorKeywordIndex));
			}
		});
	},
	...
	query: function(string,fields){
		...
		var i=0;
		var docs = [];
		for( i=0; i<this.documentsLength; i++){
		...
		var results=[];
		documentLoop:for( i=0; i<this.documentsLength; i++){
...
var stemmer = exports.stemmer = new PorterStemmer();

// break string up into tokens and stem words
var tokenizer = exports.tokenizer = DefaultTokenizer();
